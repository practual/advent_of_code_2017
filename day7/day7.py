...
def make_reverse_map(programme_map):
...
def find_root(child_to_parent_map):
    ...
    child = list(child_to_parent_map.keys())[0]
    ...
    if len(set(child_weights.values())) > 1:
        ...
        raise Exception(
            '{} is the wrong weight. Is {} but should be {}'.format(
                problem_child, problem_weight, updated_weight)
        )
...
def run():
    ...
    reverse_map = make_reverse_map(programme_map)

    root = find_root(reverse_map)
    print(root)
